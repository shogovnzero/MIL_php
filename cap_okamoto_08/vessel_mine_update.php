...
if(!empty($_POST)){
  ...
  if (!isset($error)) {
    $_SESSION['join'] = $_POST;
      // 画像アップロードのみ先行
    if($_FILES["vessel_img"]["size"]>0){
      $vessel_img = fileUpload("vessel_img","img");
      $pdo = db_conn();
      $stmt = $pdo->prepare("UPDATE cap_vessel SET vessel_img=:vessel_img WHERE vessel_id=:vessel_id");
      $stmt->bindValue(':vessel_id', $vessel_id, PDO::PARAM_INT);
      $stmt->bindValue(':vessel_img', $vessel_img, PDO::PARAM_STR);
      $status = $stmt->execute();
    }
      ...
      <form method="post" action="" enctype="multipart/form-data">
        <div class="area_top">
          <div>
            ...
            <p>画像アップロード</p>
            <input type="file" accept="image/*" name="vessel_img">
          </div>
